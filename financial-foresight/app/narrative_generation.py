...
if __name__ == "__main__":
    ...
    # Send the narrative to the backend
    backend_url = "http://127.0.0.1:5000/analyze"  # Replace with your backend URL
    data = {"narrative": narrative}
    headers = {'Content-Type': 'application/json'}
    response = requests.post(backend_url, json=data, headers=headers)

    if response.status_code == 200:
        print("Narrative sent successfully!")
    else:
        print(f"Failed to send narrative. Status code: {response.status_code}")
